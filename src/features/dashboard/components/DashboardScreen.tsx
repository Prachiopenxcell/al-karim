...
import DepartmentProgressRow from '@/features/dashboard/components/DepartmentProgressRow';
import { EnrollmentLineChart, FinancialAreaChart } from '@/features/dashboard/components/RechartsBlocks';
import type { StatTile, Department, ActivityItem } from '@/features/dashboard/types';
import { Plus, Send, TrendingUp, TrendingDown, MoreHorizontal, Users, BookOpen, CreditCard, Bed } from 'lucide-react';
...
export default function DashboardScreen({ stats, enrollment, collections, activity, departments }: DashboardScreenProps) {
  return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Chart title="Student Enrollment Trend" subtitle="Last 12 months · +15.2%">
          <div className="w-full h-64 bg-gray-50 rounded-lg p-2 pb-6">
            <EnrollmentLineChart data={enrollment} />
          </div>
        </Chart>

        <Chart title="Financial Overview" subtitle="Last 12 months · +16.2%">
          <div className="w-full h-64 bg-gray-50 rounded-lg p-2 pb-6">
            <FinancialAreaChart data={collections} />
          </div>
        </Chart>
      </div>
